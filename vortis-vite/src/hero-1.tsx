...
export function HeroLanding({ logo, navigation, title, description, announcementBanner, gradientColors, onLogin, authLoading, authError }: any) {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', width: '100vw', overflow: 'hidden', background: '#080810' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div style={{
          position: 'absolute',
          top: '-20%', left: '50%', transform: 'translateX(-50%) rotate(30deg)',
          width: '72rem', aspectRatio: '1155/678',
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 40px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {logo?.src && <img alt={logo?.alt || 'Logo'} src={logo.src} style={{ height: 32, width: 'auto' }} />}
          ...
          <button
            ...
            style={{ fontSize: 14, fontWeight: 600, color: 'rgba(255,255,255,0.65)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
          >
            Sign In →
          </button>
        </nav>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 80px)', padding: '0 24px', textAlign: 'center' }}>
        {announcementBanner && (
          <div style={{ marginBottom: 24, padding: '6px 16px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.5)', fontSize: 13, display: 'inline-block' }}>
            {announcementBanner.text}{' '}
            <a href={announcementBanner.linkHref} style={{ color: 'rgba(255,255,255,0.8)', fontWeight: 600, textDecoration: 'none' }}>{announcementBanner.linkText} →</a>
          </div>
        )}
        ...
        <p style={{ fontSize: 'clamp(15px, 2vw, 20px)', color: 'rgba(255,255,255,0.4)', maxWidth: 600, margin: '0 0 0', lineHeight: 1.6 }}>
      ...
      {showAuthModal && (
        ...
        >
          <div style={{ background: '#0d0d18', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '32px 28px', width: '100%', maxWidth: 380 }}>
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', textAlign: 'center', marginTop: 20, lineHeight: 1.8, margin: '20px 0 0' }}>
