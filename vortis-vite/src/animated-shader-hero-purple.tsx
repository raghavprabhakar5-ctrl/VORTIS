      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto animate-fade-in-up animation-delay-600">
            <p className="text-lg md:text-xl lg:text-2xl text-purple-100/80 font-light leading-relaxed">
              {subtitle}
            </p>
          </div>

          {/* CTA Buttons — purple palette */}
          {buttons && (
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 animate-fade-in-up animation-delay-800">
              {buttons.primary && (
                <button
                  onClick={buttons.primary.onClick}
                  className="px-8 py-4 bg-white hover:bg-purple-50 text-black rounded-full font-semibold text-lg transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-purple-500/25"
                >
...
// ─── Purple Shader ─────────────────────────────────────────────────────────────
// Same cloud/fbm structure as original, colours remapped to purple/violet/indigo
...
void main(void) {
  ...
  // purple/violet background cloud
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec2 p=uv;
    float d=length(p);
    // remap colour channels: R→violet, G→purple, B→indigo/blue
    col+=.00125/d*(cos(sin(i)*vec3(2,1,3))+1.);
    float b=noise(i+p+bg*1.731);
    col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
    // mix toward deep purple bg instead of amber
    col=mix(col,vec3(bg*.08,bg*.04,bg*.18),d);
  }

  // tint final output purple
  col = vec3(col.r*0.6 + col.b*0.3, col.g*0.3, col.b*0.9 + col.r*0.2);
  col += vec3(bg*0.04, bg*0.01, bg*0.12);
