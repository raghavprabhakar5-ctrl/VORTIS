...
@keyframes goldGlow{0%,100%{box-shadow:0 0 20px rgba(251,191,36,.12),0 0 50px rgba(168,85,247,.08)}50%{box-shadow:0 0 50px rgba(251,191,36,.3),0 0 100px rgba(168,85,247,.18)}}
@keyframes silverGlow{0%,100%{box-shadow:0 0 20px rgba(148,163,184,.1)}50%{box-shadow:0 0 40px rgba(148,163,184,.25)}}
@keyframes platinumGlow{0%,100%{box-shadow:0 0 20px rgba(6,182,212,.12)}50%{box-shadow:0 0 50px rgba(6,182,212,.3)}}
...
@keyframes neonPulse{0%,100%{text-shadow:0 0 7px rgba(139,92,246,.5),0 0 20px rgba(139,92,246,.3)}50%{text-shadow:0 0 14px rgba(139,92,246,.8),0 0 40px rgba(139,92,246,.5)}}
@keyframes morphBg{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}
...
function FloatingParticles() {
  const particles = useRef(Array.from({ length: 35 }, (_, i) => ({
    id: i, x: Math.random() * 100, y: Math.random() * 100,
    size: Math.random() * 2.5 + 0.5,
    duration: Math.random() * 20 + 15, delay: Math.random() * 10,
    ...
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      {particles.map(p => (
        <div key={p.id} style={{
          ...
          animation: `float ${p.duration}s ease-in-out ${p.delay}s infinite`, opacity: 0.5,
        }} />
      ))}
    </div>
  );
...
function CosmicBg() {
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      {[
        { a: "orb1 28s ease-in-out infinite", t: "5%", l: "10%", w: 700, c: "rgba(124,58,237,0.055)", b: 120 },
        { a: "orb2 35s ease-in-out infinite 5s", t: "40%", r: "5%", w: 600, c: "rgba(168,85,247,0.045)", b: 140 },
        { a: "orb3 40s ease-in-out infinite 12s", bt: "10%", l: "30%", w: 520, c: "rgba(6,182,212,0.03)", b: 110 },
        { a: "orb4 32s ease-in-out infinite 8s", t: "60%", l: "5%", w: 450, c: "rgba(99,102,241,0.035)", b: 130 },
      ].map((o, i) => (
        <div key={i} style={{
...
function Nav() {
  ...
  return (
    <nav style={{
      ...
      background: scrolled ? "rgba(3,3,10,0.95)" : "transparent",
      ...
      <a href="#" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 99, background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.4)", color: "#c4b5fd" }}>AI</span>
      </a>
      ...
      <a href="#pricing" style={{
        padding: "9px 24px", borderRadius: 99, fontSize: 13.5, fontWeight: 700,
        background: "linear-gradient(135deg,#7C3AED,#8b5cf6)", color: "#fff",
        textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 7,
        boxShadow: "0 0 24px rgba(124,58,237,0.45)",
        transition: "all 0.2s",
      }}
      onMouseEnter={e => { e.currentTarget.style.transform = "scale(1.04) translateY(-1px)"; e.currentTarget.style.boxShadow = "0 0 40px rgba(124,58,237,0.65)"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.boxShadow = "0 0 24px rgba(124,58,237,0.45)"; }}
      ><Zap size={14} /> Get Started</a>
    </nav>
  );
}
...
function HeroVisual() {
  ...
  const msgs = [
    { role: "user", text: "Analyze competitor pricing strategies" },
    { role: "ai", text: "Scanning 12 sources... Stripe charges 2.9% + 30¢, PayPal 3.49% + 49¢..." },
    { role: "user", text: "Create a comparison visualization" },
    { role: "ai", text: "Building your chart now", typing: true },
  ];
  return (
    <div style={{ width: "100%", maxWidth: 460, borderRadius: 20, background: "rgba(10,10,25,0.8)", border: "1px solid rgba(255,255,255,0.09)", overflow: "hidden", animation: "waveFloat 6s ease-in-out infinite", boxShadow: "0 40px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(124,58,237,0.12)" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.02)" }}>
        {["#ef4444","#f59e0b","#10b981"].map(c => <div key={c} style={{ width: 10, height: 10, borderRadius: "50%", background: c, opacity: 0.75 }} />)}
        <div style={{ flex: 1, height: 6, borderRadius: 3, background: "rgba(255,255,255,0.05)", margin: "0 12px" }} />
        <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "rgba(255,255,255,0.4)", fontFamily: "'JetBrains Mono',monospace" }}>
      ...
      <div style={{ padding: "16px 16px 12px", display: "flex", flexDirection: "column", gap: 12 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", justifyContent: m.role === "user" ? "flex-end" : "flex-start", opacity: step > i * 0.6 ? 1 : 0, transform: step > i * 0.6 ? "translateY(0)" : "translateY(10px)", transition: "all 0.5s ease" }}>
            {m.role === "ai" && (
              <div style={{ width: 26, height: 26, borderRadius: 8, background: "linear-gradient(135deg,#7C3AED,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <VortisLogo size={13} color="#fff" />
              </div>
            )}
            <div style={{ padding: "8px 12px", borderRadius: m.role === "user" ? "14px 14px 4px 14px" : "4px 14px 14px 14px", background: m.role === "user" ? "linear-gradient(135deg,#7C3AED,#6366f1)" : "rgba(255,255,255,0.06)", border: m.role === "ai" ? "1px solid rgba(255,255,255,0.08)" : "none", fontSize: 12.5, color: m.role === "user" ? "#e9d5ff" : "rgba(255,255,255,0.85)", maxWidth: "80%", lineHeight: 1.55 }}>
              {m.typing ? <span style={{ display: "flex", gap: 4, alignItems: "center" }}>{[0,1,2].map(d => <span key={d} style={{ width: 6, height: 6, borderRadius: "50%", background: "#a855f7", animation: `pulse 1.2s ease-in-out ${d*0.2}s infinite` }} />)}</span> : m.text}
            </div>
          </div>
        ))}
      </div>
      <div style={{ padding: "10px 16px 14px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: 8, alignItems: "center" }}>
        <div style={{ flex: 1, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", padding: "0 12px" }}>
          <span style={{ fontSize: 12, color: "rgba(255,255,255,0.22)", fontFamily: "'JetBrains Mono',monospace" }}>Ask Vortis anything…</span>
        </div>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#7C3AED,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(124,58,237,0.5)" }}>
...
function Hero() {
  ...
  useEffect(() => {
    const id = setInterval(() => setWordIdx(i => (i + 1) % CYCLE_WORDS.length), 3400);
    return () => clearInterval(id);
  }, []);
  return (
    <section style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", alignItems: "center", gap: 60, padding: "100px 80px 80px", position: "relative", zIndex: 1, maxWidth: 1340, margin: "0 auto" }}>
      <style>{`@media(max-width:900px){.hero-visual{display:none!important}}`}</style>
      <div>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 99, border: "1px solid rgba(139,92,246,0.4)", background: "rgba(139,92,246,0.09)", marginBottom: 28, animation: "fadeUp 0.6s ease both" }}>
        ...
        <p style={{ fontSize: 17, color: "rgba(255,255,255,0.55)", maxWidth: 480, lineHeight: 1.75, marginBottom: 40, animation: "fadeUp 0.7s 0.5s ease both" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 52, animation: "fadeUp 0.7s 0.65s ease both" }}>
          <a href="#pricing" style={{ padding: "14px 32px", borderRadius: 99, fontSize: 15, fontWeight: 700, background: "linear-gradient(135deg,#7C3AED,#8b5cf6)", color: "#fff", border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8, boxShadow: "0 0 40px rgba(124,58,237,0.5), 0 8px 32px rgba(124,58,237,0.3)", transition: "all 0.25s", textDecoration: "none" }}
          ...
          ><Zap size={16} /> Start Free — No card needed</a>
          <a href="#capabilities" style={{ padding: "14px 26px", borderRadius: 99, fontSize: 15, fontWeight: 600, border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.8)", background: "rgba(255,255,255,0.04)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 8, transition: "all 0.2s" }}
        ...
        <div style={{ display: "flex", gap: 32, animation: "fadeUp 0.7s 0.8s ease both", flexWrap: "wrap" }}>
      ...
      <div className="hero-visual" style={{ display: "flex", justifyContent: "center", alignItems: "center", position: "relative" }}>
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.12), transparent 70%)", filter: "blur(40px)" }} />
        <HeroVisual />
        {[
          ...
          <div key={b.text} style={{ position: "absolute", top: b.top, right: b.right, bottom: b.bottom, left: b.left, display: "flex", alignItems: "center", gap: 7, padding: "8px 14px", borderRadius: 99, background: "rgba(8,8,20,0.9)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.9)", animation: `float 4s ease-in-out ${b.delay} infinite`, boxShadow: "0 8px 32px rgba(0,0,0,0.4)", whiteSpace: "nowrap" }}>
...
// ── BRAND LOGOS ──
const BRAND_DATA = {
  Google:{multiPath:[{d:"M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z",fill:"#4285F4"},{d:"M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z",fill:"#34A853"},{d:"M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z",fill:"#FBBC05"},{d:"M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z",fill:"#EA4335"}]},
  Microsoft:{multiPath:[{d:"M1 1h10.5v10.5H1z",fill:"#F25022"},{d:"M12.5 1H23v10.5H12.5z",fill:"#7FBA00"},{d:"M1 12.5h10.5V23H1z",fill:"#00A4EF"},{d:"M12.5 12.5H23V23H12.5z",fill:"#FFB900"}]},
...
const ROW1 = ["Google","Microsoft","Apple","Meta","Stripe","Vercel","GitHub","Notion","OpenAI","Discord","Google","Microsoft"];
const ROW2 = ["GitHub","Notion","OpenAI","Discord","Google","Microsoft","Apple","Meta","Stripe","Vercel","GitHub","Notion"];
...
function LogoItem({ name }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 9, flexShrink: 0, borderRadius: 99, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", height: 44, padding: "0 20px", cursor: "default", transition: "all 0.25s" }}
    ...
    onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; e.currentTarget.style.background = "rgba(255,255,255,0.02)"; e.currentTarget.style.transform = "translateY(0)"; }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.5)", whiteSpace: "nowrap" }}>{name}</span>
    </div>
  );
}
...
function Logos() {
  ...
  return (
    <section ref={ref} style={{ padding: "64px 0", position: "relative", zIndex: 1, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
      ...
      <p style={{ textAlign: "center", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.25)", marginBottom: 24, letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "'JetBrains Mono',monospace", opacity: inView ? 1 : 0, transition: "opacity 0.8s ease" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, opacity: inView ? 1 : 0, transition: "opacity 1s 0.2s ease" }}>
        {[ROW1, ROW2].map((row, ri) => (
          <div key={ri} style={{ overflow: "hidden" }}>
            <div style={{ display: "flex", gap: 10, width: "max-content", animation: `${ri ? "marquee-r" : "marquee-l"} ${ri ? 50 : 42}s linear infinite`, willChange: "transform" }}>
              {[...row, ...row, ...row].map((n, i) => <LogoItem key={i} name={n} />)}
            </div>
          </div>
        ))}
      </div>
...
function BentoGrid() {
  ...
  return (
    <section id="capabilities" ref={ref} style={{ padding: "80px 40px", position: "relative", zIndex: 1, borderTop: "1px solid rgba(255,255,255,0.04)", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 56 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 99, border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.06)", marginBottom: 16, opacity: inView ? 1 : 0, transition: "opacity 0.6s ease" }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#c4b5fd", fontFamily: "'JetBrains Mono',monospace" }}>Everything you need</span>
        </div>
        <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(28px,4.5vw,48px)", margin: 0, letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(24px)", transition: "all 0.8s 0.1s ease" }}>
          One platform.{" "}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {features.map((f, i) => {
          const Icon = f.icon;
          const isLarge = f.size === "large" && (i === 0 || i === 5);
          return (
            <div key={f.title} style={{ gridColumn: isLarge ? "span 2" : "span 1", minHeight: 200, borderRadius: 18, padding: "28px 30px", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden", cursor: "default", transition: "all 0.3s", opacity: inView ? 1 : 0, transform: inView ? "translateY(0) scale(1)" : "translateY(30px) scale(0.97)", transitionDelay: `${i * 0.06 + 0.15}s` }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = `rgba(${f.color},0.4)`; e.currentTarget.style.background = `rgba(${f.color},0.07)`; e.currentTarget.style.transform = "translateY(-4px) scale(1.01)"; e.currentTarget.style.boxShadow = `0 20px 60px rgba(${f.color},0.15)`; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)"; e.currentTarget.style.background = "rgba(255,255,255,0.025)"; e.currentTarget.style.transform = "translateY(0) scale(1)"; e.currentTarget.style.boxShadow = "none"; }}>
...
function About() {
  ...
  const stats = [
    { icon: Users, value: "50,000+", label: "Active users worldwide", color: "124,58,237" },
    { icon: TrendingUp, value: "10B+", label: "Tokens processed monthly", color: "168,85,247" },
    { icon: Clock, value: "< 200ms", label: "Average response time", color: "6,182,212" },
    { icon: Database, value: "99.9%", label: "Uptime SLA guarantee", color: "99,102,241" },
  ];
  const team = [
    { initials: "AR", name: "Aiden Ross", role: "CEO & Co-Founder", color: "124,58,237", bg: "rgba(124,58,237,0.2)" },
    { initials: "MK", name: "Maya Kim", role: "CTO & Co-Founder", color: "168,85,247", bg: "rgba(168,85,247,0.2)" },
    { initials: "JS", name: "Jake Sullivan", role: "Head of AI Research", color: "6,182,212", bg: "rgba(6,182,212,0.2)" },
  ];
  return (
    <section id="about" ref={ref} style={{ padding: "80px 40px", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center", marginBottom: 72 }}>
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 99, border: "1px solid rgba(6,182,212,0.3)", background: "rgba(6,182,212,0.06)", marginBottom: 20, opacity: inView ? 1 : 0, transition: "opacity 0.7s ease" }}>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(6,182,212,0.9)", fontFamily: "'JetBrains Mono',monospace" }}>Our Story</span>
          </div>
          <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(26px,4vw,44px)", margin: "0 0 20px", letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-40px)", transition: "all 0.8s 0.1s ease" }}>
            Built by engineers,{" "}
            <span style={{ background: "linear-gradient(90deg,#7C3AED,#06B6D4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>for everyone.</span>
          </h2>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, marginBottom: 20, opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-30px)", transition: "all 0.8s 0.25s ease" }}>
            Vortis was founded in 2024 with a simple belief: AI tools should feel as natural as thinking itself. We were frustrated by platforms that forced you to switch between apps, lose context, and fight against interfaces that didn't understand the way humans actually work.
          </p>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, marginBottom: 32, opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-30px)", transition: "all 0.8s 0.35s ease" }}>
            So we built something better — a unified intelligence surface where chat, code, vision, research, and memory all work together seamlessly.
          </p>
          <div style={{ display: "flex", gap: 12, opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s 0.45s ease" }}>
            {team.map(m => (
              <div key={m.name} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", transition: "all 0.2s" }}
              onMouseEnter={e => { e.currentTarget.style.background = `rgba(${m.color},0.08)`; e.currentTarget.style.borderColor = `rgba(${m.color},0.3)`; }}
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: m.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: `rgb(${m.color})`, flexShrink: 0 }}>{m.initials}</div>
                <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(40px)", transition: "all 0.8s 0.2s ease" }}>
          {stats.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={s.label} style={{ borderRadius: 18, padding: "24px 20px", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden", transition: "all 0.3s", transitionDelay: `${i*0.1}s` }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = `rgba(${s.color},0.4)`; e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 16px 48px rgba(${s.color},0.15)`; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)"; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; }}>
                <div style={{ position: "absolute", top: 0, right: 0, width: 80, height: 80, background: `radial-gradient(circle, rgba(${s.color},0.12), transparent)`, pointerEvents: "none" }} />
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(${s.color},0.15)`, border: `1px solid rgba(${s.color},0.3)`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                  <Icon size={18} style={{ color: `rgb(${s.color})` }} />
                </div>
                <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: 26, color: "#fff", marginBottom: 6 }}>{s.value}</div>
                <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.45)", lineHeight: 1.5 }}>{s.label}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* Mission banner */}
      <div style={{ borderRadius: 24, padding: "48px 56px", background: "rgba(124,58,237,0.07)", border: "1px solid rgba(124,58,237,0.2)", position: "relative", overflow: "hidden", textAlign: "center", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.8s 0.5s ease" }}>
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 60% 80% at 50% 50%, rgba(124,58,237,0.06), transparent)", pointerEvents: "none" }} />
        <div style={{ fontSize: 36, marginBottom: 16 }}>🚀</div>
        <h3 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 800, fontSize: "clamp(20px,3vw,32px)", margin: "0 0 16px", color: "#fff", letterSpacing: "-0.02em" }}>Our Mission</h3>
        <p style={{ fontSize: 17, color: "rgba(255,255,255,0.6)", maxWidth: 640, margin: "0 auto", lineHeight: 1.75 }}>
          To give every person on Earth access to the highest quality AI reasoning — regardless of their background, resources, or technical expertise. Intelligence should be a right, not a privilege.
        </p>
      </div>
    </section>
  );
    ...
    <section id="howitworks" ref={ref} style={{ padding: "80px 40px", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          ...
          <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(28px,4.5vw,48px)", margin: 0, letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s 0.1s ease" }}>
            How Vortis{" "}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, position: "relative" }}>
          ...
          {steps.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={s.num} style={{ textAlign: "center", position: "relative", zIndex: 1, padding: "28px 24px 32px", borderRadius: 20, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(32px)", transition: `all 0.8s ${i * 0.15 + 0.2}s ease` }}>
...
// ── SHOWCASE (fixed equal height with orbit visual) ──
const CAPS = [
  { id: "chat", name: "AI Chat", icon: MessageSquare, color: "124,58,237", tagline: "Reason and create at the speed of thought", description: "Advanced conversational AI that understands nuance, remembers context, and responds with unprecedented accuracy across any topic." },
  { id: "code", name: "Coding", icon: Code2, color: "6,182,212", tagline: "Write, refactor, review at principal quality", description: "Full-stack development assistant that writes, debugs, refactors, and explains code across every major language and framework." },
  { id: "image", name: "Image Gen", icon: ImageIcon, color: "168,85,247", tagline: "Studio-quality visuals in milliseconds", description: "Generate photorealistic images, illustrations, concept art, and designs from natural language descriptions." },
  { id: "search", name: "Web Search", icon: Globe, color: "124,58,237", tagline: "Real-time internet with perfect attribution", description: "Live search across the entire web with intelligent source attribution, fact verification, and concise summarization." },
  { id: "memory", name: "Memory", icon: Brain, color: "168,85,247", tagline: "Persistent context that never forgets", description: "Cross-session memory that retains your projects, preferences, and conversations so you never have to repeat yourself." },
  { id: "research", name: "Deep Research", icon: Microscope, color: "6,182,212", tagline: "Autonomous agents synthesizing 50+ sources", description: "Autonomous research agents that browse, synthesize, and produce comprehensive reports from dozens of sources in minutes." },
];

function OrbitVisual({ cap }) {
  ...
  const satellites = [
    { label: "Context", r: 85, startAngle: 0 },
    { label: "Memory", r: 85, startAngle: 90 },
    { label: "Output", r: 85, startAngle: 180 },
    { label: "Search", r: 85, startAngle: 270 },
    { label: "Vision", r: 140, startAngle: 45 },
    { label: "Code", r: 140, startAngle: 135 },
  ];

  const cx = 200, cy = 200;
  const [r, g, b] = cap.color.split(",");

  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
      <svg width="400" height="400" viewBox="0 0 400 400" style={{ position: "absolute", inset: 0, margin: "auto" }}>
        <defs>
          <radialGradient id={`grd-${cap.id}`} cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor={`rgba(${cap.color},0.2)`} />
            <stop offset="100%" stopColor="transparent" />
          </radialGradient>
        </defs>
        {/* Orbit rings */}
        <circle cx={cx} cy={cy} r="85" fill="none" stroke={`rgba(${cap.color},0.12)`} strokeWidth="1" strokeDasharray="4 8" />
        <circle cx={cx} cy={cy} r="140" fill="none" stroke={`rgba(${cap.color},0.08)`} strokeWidth="1" strokeDasharray="4 12" />
        {/* Glow core */}
        <circle cx={cx} cy={cy} r="40" fill={`url(#grd-${cap.id})`} />
        <circle cx={cx} cy={cy} r="28" fill={`rgba(${cap.color},0.15)`} stroke={`rgba(${cap.color},0.5)`} strokeWidth="1" />
        <circle cx={cx} cy={cy} r="16" fill={`rgba(${cap.color},0.35)`}>
          <animate attributeName="r" values="14;18;14" dur="2s" repeatCount="indefinite" />
        </circle>
        {/* Satellites */}
        {satellites.map((sat, i) => {
          const a = ((sat.startAngle + angle * (i % 2 === 0 ? 1 : -0.6)) * Math.PI) / 180;
          const sx = cx + sat.r * Math.cos(a);
          const sy = cy + sat.r * Math.sin(a);
          return (
            <g key={sat.label}>
              <line x1={cx} y1={cy} x2={sx} y2={sy} stroke={`rgba(${cap.color},0.15)`} strokeWidth="1" />
              <circle cx={sx} cy={sy} r="18" fill={`rgba(${cap.color},0.12)`} stroke={`rgba(${cap.color},0.5)`} strokeWidth="1" />
              <circle cx={sx} cy={sy} r="5" fill={`rgba(${cap.color},0.8)`}>
      ...
      {/* Center icon */}
      <div style={{ position: "relative", zIndex: 2, width: 56, height: 56, borderRadius: "50%", background: `rgba(${cap.color},0.2)`, border: `2px solid rgba(${cap.color},0.6)`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 40px rgba(${cap.color},0.3)` }}>
        {(() => { const Icon = cap.icon; return <Icon size={22} style={{ color: `rgb(${cap.color})` }} />; })()}
      </div>
      {/* Satellite labels */}
      {satellites.map((sat, i) => {
        const a = ((sat.startAngle + angle * (i % 2 === 0 ? 1 : -0.6)) * Math.PI) / 180;
        const sx = 200 + sat.r * Math.cos(a);
        const sy = 200 + sat.r * Math.sin(a);
        return (
          <div key={`lbl-${sat.label}`} style={{ position: "absolute", left: sx, top: sy, transform: "translate(-50%, -50%)", zIndex: 3, pointerEvents: "none" }}>
            <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 99, whiteSpace: "nowrap", background: `rgba(${cap.color},0.2)`, color: `rgb(${cap.color})`, border: `1px solid rgba(${cap.color},0.5)`, display: "block" }}>{sat.label}</span>
          </div>
        );
      })}
    </div>
...
function Showcase() {
  ...
  return (
    <section id="capabilities-detail" ref={ref} style={{ padding: "80px 40px", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1 }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 99, border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.06)", marginBottom: 16, opacity: inView ? 1 : 0, transition: "opacity 0.7s ease" }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#c4b5fd", fontFamily: "'JetBrains Mono',monospace" }}>Intelligence Graph</span>
          </div>
          <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(28px,4.5vw,48px)", margin: 0, letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(24px)", transition: "all 0.8s 0.1s ease" }}>
            Every mode.{" "}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 16, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {CAPS.map((c, i) => {
              ...
              return (
                <button key={c.id} onClick={() => setActive(i)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 14, textAlign: "left", cursor: "pointer", border: isActive ? `1px solid rgba(${c.color},0.5)` : "1px solid rgba(255,255,255,0.05)", background: isActive ? `rgba(${c.color},0.15)` : "rgba(255,255,255,0.02)", transition: "all 0.2s", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-30px)", transitionDelay: `${i * 0.06 + 0.2}s` }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: isActive ? `rgba(${c.color},0.3)` : `rgba(${c.color},0.1)`, border: `1px solid rgba(${c.color},0.3)`, flexShrink: 0 }}>
                    <Icon size={16} style={{ color: `rgb(${c.color})` }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: isActive ? "#fff" : "rgba(255,255,255,0.6)" }}>{c.name}</div>
                    {isActive && <div style={{ fontSize: 10.5, color: `rgba(${c.color},0.85)`, marginTop: 2, lineHeight: 1.4 }}>{c.tagline}</div>}
                  </div>
                </button>
              );
            })}
          ...
          {/* Main panel */}
          <div key={cap.id} style={{ borderRadius: 20, overflow: "hidden", background: "rgba(5,5,18,0.95)", border: "1px solid rgba(255,255,255,0.07)", minHeight: 460, display: "flex", flexDirection: "column", opacity: inView ? 1 : 0, transition: "all 0.5s ease" }}>
            {/* Scan line */}
            <div style={{ height: 2, background: `linear-gradient(90deg, transparent, rgba(${cap.color},0.5), transparent)`, animation: "scanLine 4s linear infinite", flexShrink: 0 }} />
            {/* Orbit visual */}
            <div style={{ flex: 1, position: "relative", minHeight: 380, backgroundImage: "radial-gradient(circle, rgba(255,255,255,0.4) 1px, transparent 1px)", backgroundSize: "28px 28px", backgroundPosition: "center", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 70% 70% at 50% 50%, rgba(${cap.color},0.07), transparent)`, pointerEvents: "none" }} />
              <div style={{ width: "100%", height: 380 }}>
                <OrbitVisual cap={cap} />
              </div>
            </div>
            {/* Description */}
            <div style={{ padding: "24px 28px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
              <h3 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 18, margin: "0 0 8px", color: "#fff" }}>{cap.name}</h3>
              <p style={{ fontSize: 14, color: "rgba(255,255,255,0.55)", lineHeight: 1.7, margin: 0 }}>{cap.description}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section ref={ref} style={{ padding: "80px 0", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1, overflow: "hidden" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 48, padding: "0 40px" }}>
        <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(26px,4vw,44px)", margin: 0, letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transition: "all 0.7s ease" }}>
          Loved by{" "}
      ...
      <div style={{ display: "flex", gap: 16, width: "max-content", animation: "marquee-l 60s linear infinite", willChange: "transform", opacity: inView ? 1 : 0, transition: "opacity 1s 0.2s ease" }}>
        {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
          <div key={i} style={{ width: 300, flexShrink: 0, padding: "24px", borderRadius: 16, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)" }}>
...
function FaqItem({ faq, index, inView }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : `translateX(${index % 2 === 0 ? -40 : 40}px)`, transition: `all 0.6s ${index * 0.07}s ease` }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 4px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
        <span style={{ fontSize: 15, fontWeight: 500, color: open ? "#fff" : "rgba(255,255,255,0.75)", transition: "color 0.2s" }}>{faq.q}</span>
        <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, marginLeft: 16, border: `1px solid ${open ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.1)"}`, background: open ? "rgba(139,92,246,0.15)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transform: open ? "rotate(45deg)" : "rotate(0deg)", transition: "all 0.28s", boxShadow: open ? "0 0 12px rgba(139,92,246,0.3)" : "none" }}>
      ...
      <div style={{ maxHeight: open ? 220 : 0, overflow: "hidden", transition: "max-height 0.38s ease" }}>
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", lineHeight: 1.75, paddingBottom: 22, paddingRight: 48, margin: 0 }}>{faq.a}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <section id="faq" ref={ref} style={{ padding: "80px 40px", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1 }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 99, border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.06)", marginBottom: 16, opacity: inView ? 1 : 0, transition: "all 0.7s ease" }}>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#c4b5fd", fontFamily: "'JetBrains Mono',monospace" }}>FAQ</span>
          </div>
          <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(26px,4vw,44px)", margin: 0, letterSpacing: "-0.03em", color: "#fff", opacity: inView ? 1 : 0, transition: "all 0.8s 0.1s ease" }}>
            Frequently Asked{" "}
...
function CTA() {
  ...
  return (
    <section ref={ref} style={{ padding: "100px 40px", borderTop: "1px solid rgba(255,255,255,0.04)", position: "relative", zIndex: 1, textAlign: "center" }}>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", position: "relative", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.8s ease" }}>
        <div style={{ fontSize: 52, marginBottom: 20 }}>✦</div>
        <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 900, fontSize: "clamp(32px,6vw,62px)", margin: "0 0 20px", letterSpacing: "-0.04em", lineHeight: 1.05, color: "#fff" }}>
          Start thinking{" "}
        ...
        <a href="#pricing" style={{ padding: "16px 40px", borderRadius: 99, fontSize: 16, fontWeight: 700, background: "linear-gradient(135deg,#7C3AED,#8b5cf6)", color: "#fff", border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8, boxShadow: "0 0 60px rgba(124,58,237,0.55)", transition: "all 0.25s", textDecoration: "none" }}
        onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px) scale(1.03)"; e.currentTarget.style.boxShadow = "0 0 80px rgba(124,58,237,0.7)"; }}
        onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0) scale(1)"; e.currentTarget.style.boxShadow = "0 0 60px rgba(124,58,237,0.55)"; }}
        ><Zap size={18} /> Get Started Free</a>
      </div>
    </section>
  );
}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 60, marginBottom: 48, alignItems: "start" }}>
          <div>
            ...
            <p style={{ fontSize: 13.5, color: "rgba(255,255,255,0.3)", maxWidth: 280, lineHeight: 1.7, margin: "0 0 20px" }}>The intelligence platform for the world's most ambitious professionals.</p>
          </div>
          <div style={{ display: "flex", gap: 56, flexWrap: "wrap" }}>
            {[
              ...
              <div key={col.title}>
                ...
                {col.links.map(l => <a key={l} href="#" style={{ display: "block", fontSize: 13.5, color: "rgba(255,255,255,0.35)", marginBottom: 9, textDecoration: "none", transition: "color 0.15s" }}
